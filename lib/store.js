...
export const BUNDLED_GUEST = path.join(APP_ROOT, "guests", "omnios", "omnios.img");
...
export function createVM(opts = {}) {
  ...
  let floppy = opts.floppy ?? "omnios";
  if (floppy === "omnios") floppy = BUNDLED_GUEST;
  else if (floppy === "none" || floppy === "no") floppy = null;
  else if (floppy) floppy = path.resolve(String(floppy));
  if (floppy && !fs.existsSync(floppy)) throw new Error(`floppy image not found: ${floppy}`);
  ...
  const vm = {
    id: newId(),
    name,
    guestOs: opts.guestOs || (floppy === BUNDLED_GUEST ? "OmniOS 1.0" : cdrom ? "Custom (CD-ROM)" : "Other"),
