...
import { isBundledApp, devRoot } from "./assets.js";
...
export async function startVM(vm) {
  ...
  const spawnArgs = await runnerSpawnArgs(cur.id);
...
/* how to re-launch this same program in "runner" mode: as a single-file
 * executable we re-exec ourselves with just the args; from source we pass
 * the entry script to node. */
async function runnerSpawnArgs(id) {
  if (await isBundledApp()) return ["run", "--id", id];
  let entry = process.argv[1] ? path.resolve(process.argv[1]) : null;
  if (!entry || path.basename(entry) !== "omnivm.js" || !fs.existsSync(entry)) {
    entry = path.join(devRoot(), "bin", "omnivm.js");
  }
  return [entry, "run", "--id", id];
}

export async function powerOffVM(vm) {
  const cur = refresh(vm);
  if (!isRunning(cur)) { cur.state = "off"; persist(cur); return cur; }
  await ctlRequest(cur, { cmd: "poweroff" }, 120000);
...
export async function suspendVM(vm) {
  ...
  await ctlRequest(cur, { cmd: "suspend" }, 150000);
...
export async function resetVM(vm) {
  const cur = refresh(vm);
  if (!isRunning(cur)) throw new Error(`"${cur.name}" is not powered on`);
  await ctlRequest(cur, { cmd: "reset" }, 120000);
  return cur;
}
...
export const snapshot = {
  ...
  async take(vm, name) {
    ...
    const res = await ctlRequest(cur, { cmd: "snapshot-save", name }, 150000);
    return res;
  },
  async restore(vm, name) {
    const cur = refresh(vm);
    if (!isRunning(cur)) throw new Error(`"${cur.name}" must be powered on to revert`);
    return ctlRequest(cur, { cmd: "snapshot-restore", name }, 150000);
  },
