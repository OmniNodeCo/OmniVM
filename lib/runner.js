...
import {
  ...
  snapshotsDir, diskImage, isBundledFloppy,
} from "./store.js";
...
let screenDirty = false, serialHistory = ""; // latin1 ring buffer, replayed to new clients
...
/* Recent serial output, replayed to newly attached clients so consoles and
 * tests see output that was printed before they attached (tmux-style). */
const SERIAL_HISTORY_MAX = 64 * 1024;

function onSerialByte(byte) {
  const buf = Buffer.from([byte]);
  serialHistory += buf.toString("latin1");
  if (serialHistory.length > SERIAL_HISTORY_MAX * 2) {
    serialHistory = serialHistory.slice(-SERIAL_HISTORY_MAX);
  }
  try {
  ...
  broadcast({ evt: "serial", data: buf.toString("base64") });
...
function persistDisks(cb = () => {}) {
  ...
  if (fda && vm.floppy && !isBundledFloppy(vm) && typeof fda.get_buffer === "function") {
    pending++;
    fda.get_buffer(buf => {
...
async function bootInstance() {
  const opts = {
    bios: { buffer: ab(await assetBuffer("seabios.bin")) },
    vga_bios: { buffer: ab(await assetBuffer("vgabios.bin")) },
    wasm_fn: async env => {
      const bytes = await assetBuffer("v86.wasm");
  ...
  if (vm.floppy && isBundledFloppy(vm)) opts.fda = { buffer: ab(await assetBuffer("omnios.img")) };
  else if (vm.floppy && fs.existsSync(vm.floppy)) opts.fda = { buffer: ab(fs.readFileSync(vm.floppy)) };
...
async function main() {
  ...
  const server = net.createServer(sock => {
    ...
    // replay recent console output so late attachers see prior output
    if (serialHistory.length) {
      send(sock, { evt: "serial", data: Buffer.from(serialHistory, "latin1").toString("base64") });
    }
  });
