...
async function main() {
  ...
  switch (cmd) {
    ...
    case "create": {
      ...
      console.log(green(`✔ created VM "${vm.name}"`) + dim(` (${vm.id}) — ${store.fmtMB(vm.memMB)} RAM, ${vm.diskMB ? store.fmtMB(vm.diskMB) + " disk, " : ""}${vm.floppy === store.BUNDLED_GUEST ? "OmniOS floppy" : vm.floppy ? "floppy " + vm.floppy : "no floppy"}`));
    ...
    case "config": {
      ...
      if (!Object.keys(o).length) {
        ...
        for (const k of ["memMB", "diskMB", "floppy", "cdrom", "boot", "guestOs", "notes"]) {
          const v = k === "floppy" && cur[k] === store.BUNDLED_GUEST ? "omnios (bundled)" : cur[k];
      ...
      if (o.floppy !== undefined) {
        if (o.floppy === "omnios") patch.floppy = store.BUNDLED_GUEST;
    ...
    case "doctor": {
      ...
      try { ok(`node ${process.version}`); } catch { bad("node missing"); }
      fs.existsSync(store.BIOS) ? ok("BIOS (SeaBIOS) found") : bad(`BIOS missing at ${store.BIOS}`);
      fs.existsSync(new URL("../node_modules/v86/build/v86.wasm", import.meta.url).pathname)
        ? ok("v86 engine (WASM) found")
        : bad("v86 engine missing — run: npm install");
      fs.existsSync(store.BUNDLED_GUEST) ? ok("bundled guest OS (OmniOS floppy) found") : bad("OmniOS floppy missing — run: make -C guests/omnios");
      store.ensureDirs();
