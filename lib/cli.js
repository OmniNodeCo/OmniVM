...
${bold("commands:")}
  (no command)            launch the graphical Workstation (canvas GUI)
  web|gui|serve [--port N]
...
${bold("keys in the terminal TUI (omnivm tui):")}  ↑/↓ select · Enter console · P power on · O power off
...
async function main() {
  ...
  if (!cmd || cmd === "web" || cmd === "gui" || cmd === "serve") {
    const portFlag = rest.indexOf("--port");
    const port = portFlag >= 0 ? parseInt(rest[portFlag + 1], 10) || 0
      : parseInt(process.env.OMNIVM_PORT || "8080", 10);
    const { startWebServer } = await import("./web/server.js");
    const { server, port: actualPort, urls } = await startWebServer({
      port: port || 0,
      openBrowser: !!process.stdout.isTTY && process.env.OMNIVM_NO_OPEN !== "1",
    });
    console.log("");
    console.log(bold("  OmniVM Workstation — graphical interface"));
    console.log(dim(`  running at  http://localhost:${actualPort}`));
    for (const u of urls) if (!u.includes("127.0.0.1")) console.log(dim(`  also at     ${u}`));
    console.log(dim("  press Ctrl-C to stop (your VMs keep running)"));
    console.log("");
    process.on("SIGINT", () => { server.close(() => process.exit(0)); setTimeout(() => process.exit(0), 500); });
    return;
  }
