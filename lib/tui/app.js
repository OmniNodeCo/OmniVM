...
class Workstation {
        ...
        if (vm && vm.state === "off") {
          this.askPrompt(`Clone "${vm.name}" as:`, vm.name + "-clone", val => {
            this.act(`cloning ${vm.name}`, async () => {
              if (val && val.trim() && val.trim() !== vm.name + "-clone") {
                const c = store.cloneVM(vm);
                c.name = store.sanitizeName(val.trim());
                store.saveVM(c);
  ...
  drawSummary(L) {
    ...
    rows.push(["Floppy", vm.floppy ? (store.isBundledFloppy(vm) ? "OmniOS 1.0 (bundled)" : vm.floppy.replace(process.env.HOME, "~")) : "empty", ST.value]);
